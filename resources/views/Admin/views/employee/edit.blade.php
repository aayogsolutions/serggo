                    ...
                    <div class="col-md-6">
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" value="{{$employee['number']}}" required name="phone" class="form-control" id="phone"
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-control w-100" name="role_id">
                                        ...
                                        @foreach($adminRoles as $role)
                                            <option
                                                value="{{$role->id}}" {{$role['id']==$employee['role_id']?'selected':''}}>{{$role->name}}</option>
                                        @endforeach
                                </select>
                            </div>
                            <div class="col-md-12">
                                ...
                                <select name="identity_type" class="form-control">
                                    ...
                                    <option value="aadhar_card" {{$employee['identity_type'] == 'aadhar_card' ? 'selected' : ''}}>{{translate('Aadhar Card')}}</option>
                                    <option value="pan_card" {{$employee['identity_type'] == 'pan_card' ? 'selected' : ''}}>{{translate('Pan Card')}}</option>
                        ...
                        <div>
                            ...
                            <div class="product--coba">
                                <div class="row g-2" id="coba">
                                    @if(is_array(json_decode($employee->identity_image)))
                                        @foreach(json_decode($employee->identity_image) as $identification_image)
                                            <div class="two__item w-50">
                                                <div class="max-h-140px existing-item">
                                                    <img src="{{asset('Images/EmployeeIdentity/').'/'.$identification_image}}" alt="{{ translate('identity_image') }}">
                                                </div>
                                            </div>
                                        @endforeach
                                    @endif
    ...
    $(function () {
        $("#coba").spartanMultiImagePicker({
            ...
            onExtensionErr: function (index, file) {
                // <?php
                //     flash()->error("Please only input png or jpg type file");
                // ?>
            },
            onSizeErr: function (index, file) {
                // <?php
                //     flash()->error("File size too big");
                // ?>
            }
        });
    });
