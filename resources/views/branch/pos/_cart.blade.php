<div class="d-flex flex-row cart--table-scroll">
    <div class="table-responsive">
        ...
        $totalTax = 0;
        $updatedTotalTax=0;
        $vatStatus = \App\CentralLogics\Helpers_get_business_settings('product_vat_tax_status') === 'included' ? 'included' : 'excluded';
        ?>
        @if(session()->has('cart') && count( session()->get('cart')) > 0)
            ...
            @foreach(session()->get('cart') as $key => $cartItem)
                @if(is_array($cartItem))
                    ...
                    $totalTax += \App\CentralLogics\Helpers_tax_calculate($product, $cartItem['price']) * $cartItem['quantity'];
                    $updatedTotalTax += $vatStatus === 'included' ? 0 : \App\CentralLogics\Helpers_tax_calculate($product, $cartItem['price']) * $cartItem['quantity'];
                    ?>
                    <tr>
                        <td>
                        ...
                        <td class="text-center px-0 py-1">
                            <div class="btn text-left">
                                {{ Helpers_set_symbol($product_subtotal) }}
                            </div>
                        </td>
                        <td class="align-items-center text-center">
...
if (session()->get('order_type') == 'home_delivery'){
    ...
    $delivery_type = \App\CentralLogics\Helpers_get_business_settings('delivery_management');
    if ($delivery_type['status'] == 1){
        $delivery_charge = \App\CentralLogics\Helpers_get_delivery_charge($distance);
    }else{
        $delivery_charge = \App\CentralLogics\Helpers_get_business_settings('delivery_charge');
    }
}else{
    $delivery_charge = 0;
}
...
<div class="box p-3">
    <dl class="row">
        ...
        <dd class="col-sm-6 text-right">{{ Helpers_set_symbol($subtotal) }}</dd>
        <dt class="col-sm-6">{{translate('product')}} {{translate('discount')}}:
        </dt>
        <dd class="col-sm-6 text-right"> - {{ Helpers_set_symbol(round($discountAmount,2)) }}</dd>
        ...
        <dd class="col-sm-6 text-right">
            <button class="btn btn-sm" type="button" data-toggle="modal" data-target="#add-discount"><i
                    class="tio-edit"></i>
            </button> - {{ Helpers_set_symbol($extraDiscount) }}</dd>

        <dt class="col-sm-6">{{translate('Delivery Charge')}} :</dt>
        <dd class="col-sm-6 text-right">{{ Helpers_set_symbol(round($delivery_charge, 2)) }}</dd>
        <dt class="col-12">
            <hr class="mt-0">
        </dt>
        <dt class="col-sm-6">{{translate('total')}} :</dt>
        <dd class="col-sm-6 text-right h4 b">{{ Helpers_set_symbol(round($total+$updatedTotalTax+$delivery_charge, 2)) }}</dd>
    </dl>
    <div>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <select name="type" class="form-control" id="discount_type_select">
                            <option
                                ...
                                ({{\App\CentralLogics\Helpers_currency_symbol()}})
                            </option>
                            <option
