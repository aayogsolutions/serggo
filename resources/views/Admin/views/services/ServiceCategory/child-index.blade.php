...
@section('title', translate('Add new service child category'))
...
<div class="content container-fluid">
    <div class="page-header">
        <h1 class="page-header-title">
            ...
            <span>
                {{translate('Service_child_category_setup')}}
            </span>
        </h1>
    </div>
    <div class="row gx-2 gx-lg-3">
        <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('admin.service.category.child-store')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <br>
                                <div class="lang_form" id="-form">
                                    <label class="form-label" for="exampleFormControlInput1">{{translate('child_category')}} {{translate('name')}}</label>
                                ...
                                <div class="">
                                    <div class="form-group">
                                        <label class="form-label"
                                            for="exampleFormControlSelect1">{{translate('sub')}} {{translate('category')}}
                                            <span class="input-label-secondary">*</span></label>
                                        <select id="exampleFormControlSelect1" name="parent_id" class="form-control" required>
                                            @foreach(\App\Models\ServiceCategory::where(['position'=>1])->get() as $subcategory)
                            ...
                            <div class="col-md-6">
                                ...
                                <label class="form-label text-capitalize">{{ translate('child category image') }}</label>
        ...
        <div class="card">
            <div class="card-header border-0">
                <div class="card--header">
                    <h5 class="card-title">{{translate('Service Child Category Table')}} <span class="badge badge-soft-secondary">{{ $categories->total() }}</span> </h5>
                ...
                <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                    <thead class="thead-light">
                        <tr>
                            ...
                            <th>{{translate('sub')}} {{translate('category')}}</th>
                            <th>{{translate('child_category')}}</th>
                    ...
                    <tbody id="set-rows">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <label class="toggle-switch">
                                    <input type="checkbox" class="toggle-switch-input status-change-alert" id="stocksCheckbox{{ $category->id }}"
                                        data-route="{{ route('admin.service.category.status', [$category->id, $category->status == 1 ? 0 : 1]) }}"
                            ...
                            <td>
                                ...
                                <form action="{{route('admin.service.category.delete',[$category['id']])}}"
