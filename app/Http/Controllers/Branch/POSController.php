...
class POSController extends Controller
{
    ...
    public function index(Request $request): View|Factory|Application
    {
        ...
        $products = $this->product->
            ...
            ->active()->latest()->paginate(Helpers_getPagination());

        $branch = $this->branch->find(auth('branch')->id());
        return view('branch.pos.index', compact('categories', 'products', 'category', 'keyword', 'branch', 'users'));
    }
    ...
    public function quickView(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('branch.pos._quick-view-data', compact('product', 'discount'))->render(),
        ]);
    }
    ...
    public function variantPrice(Request $request): array
    {
        ...
        return array('price' => Helpers_set_symbol(($price * $request->quantity)));
    }
    ...
    public function discountCalculation($product, $price): float
    {
        ...
        $categoryDiscount = Helpers_category_discount_calculate($categoryId, $price);
        $productDiscount = Helpers_discount_calculate($product, $price);
    ...
    public function addToCart(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $taxOnProduct = Helpers_tax_calculate($product, $price);
    ...
    public function order_list(Request $request): View|Factory|Application
    {
        ...
        $orders = $query->latest()->paginate(Helpers_getPagination())->appends($queryParam);

        return view('branch.pos.order.list', compact('orders', 'search', 'startDate', 'endDate'));
    }
    ...
    public function order_details($id): View|Factory|RedirectResponse|Application
    {
        ...
        $deliverymanList = $this->deliveryman->where(['is_active'=>1])
            ...
            ->get();
        if (isset($order)) {
            return view('branch.order.order-view', compact('order', 'deliverymanList'));
        } else {
    ...
    public function place_order(Request $request): RedirectResponse
    {
        ...
        if ($orderType == 'home_delivery'){
            ...
            $deliveryType = Helpers_get_business_settings('delivery_management');
            if ($deliveryType['status'] == 1){
                $deliveryCharge = Helpers_get_delivery_charge($distance);
            }else{
                $deliveryCharge = Helpers_get_business_settings('delivery_charge');
            }
        ...
        foreach ($cart as $c) {
            if (is_array($c)) {
                ...
                if ($product) {
                    ...
                    $product = Helpers_product_data_formatting($product);
                    ...
                    $or_d = [
                        ...
                        'quantity' => $c['quantity'],
                        'price' => $price,
                        'tax_amount' => Helpers_tax_calculate($product, $price),
                        'discount_on_product' => $discount,
                        'discount_type' => $discountType,
                        'variant' => json_encode($c['variant']),
                        'variation' => json_encode($c['variations']),
                        'vat_status' => Helpers_get_business_settings('product_vat_tax_status') === 'included' ? 'included' : 'excluded',
                        'created_at' => now(),
                        'updated_at' => now()
                    ];
    ...
    public function generateInvoice($id): JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('branch.pos.order.invoice', compact('order'))->render(),
        ]);
    }
    ...
    public function addDeliveryInfo(Request $request): JsonResponse
    {
        ...
        if ($validator->fails()) {
            return response()->json(['errors' => Helpers_error_processor($validator)], 200);
        }
        ...
        $mapApiKey = Helpers_get_business_settings('map_api_server_key');
        ...
        return response()->json([
            'data' => $address,
            'view' => view('admin.pos._address', compact('address'))->render(),
        ]);
    }
